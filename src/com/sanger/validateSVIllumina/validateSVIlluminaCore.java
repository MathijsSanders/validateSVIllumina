...
public class validateSVIlluminaCore {

	private String header = null;
	private final Pattern pattern = Pattern.compile("[\\]\\[](.*):");
	private final Pattern locPat = Pattern.compile(":([0-9]+)");
	
	public validateSVIlluminaCore(File inputBam, File snifflesFile, String outputFile, int sw, int dd, int threads) {
		System.out.println("Loading SVs...");
		var svList = retrieveStructuralVariants(snifflesFile, dd);
		...
		try {
			writeResults(svList, outputFile);
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(-11);
		}
